...
// ---------------------------------------------------------------------------
// Pass config path into the SCM-launched service_main via a static.
// ---------------------------------------------------------------------------
...
/// Entered from `main` when `--service` is passed. Stores the config path,
/// connects to the SCM dispatcher, and blocks until the service stops.
pub fn run_as_service(config_path: &str) -> Result<()> {
    // Store config for the service_main callback.
    SERVICE_CONFIG
...
/// Open the optical service with the requested access.
...
pub fn install(config_path: &str) -> Result<()> {
    ...
    if manager
        ...
        .is_ok()
    {
        bail!("service '{SERVICE_NAME}' is already installed; run `optical uninstall` first");
    }
    ...
    println!("Run `optical start` to start it now, or use `services.msc`.");
    Ok(())
}

pub fn uninstall() -> Result<()> {
    let service = open_service(ServiceAccess::DELETE | ServiceAccess::STOP | ServiceAccess::QUERY_STATUS)?;
