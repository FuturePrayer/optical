...
后量子加密隧道转发工具 (ML-KEM + ML-DSA + PSK),提供两个可执行文件:

- **`optical`** — 隧道节点:监听本地端口,通过加密多路复用隧道转发到远端节点。可被配置中心纳管。
- **`optical-center`** — 配置中心:集中管理多个节点的转发配置,内置 Web 管理后台(REST/SSE API + 嵌入式 React UI)。同时具备节点的全部功能(双重身份)。

两者使用相同的后量子加密栈:传输层 ChaCha20-Poly1305 AEAD,握手层 ML-KEM-768 密钥封装 + ML-DSA-65 数字签名 + PSK 预共享密钥三重认证。**不依赖也不引入常规 TLS**(详见 [AGENTS.md](AGENTS.md) 的"加密设计原则")。
...
```
浏览器 ──HTTP/REST/SSE──> optical-center (Web 管理后台)
                                │ /api/config/push (表单提交)
                                ↓
                          NodeRegistry (白名单 + 节点配置,持久化)
                                │ ConfigPush 帧(AEAD 加密长连接)
                                ↓
                    optical 节点 (CenterClient)
                                │ ConfigManager 热应用
                                ↓
                          forwarder 监听本地端口 → 经隧道转发
```

`optical-center` 通过 Web UI 集中下发 `forwarders` 配置到多个 `optical` 节点,节点身份基于 ML-DSA-65 公钥指纹(SHA-256),配置变更热生效(无需重启进程)。详见 **[配置中心使用文档](docs/config-center.md)**。
...
从 [GitHub Releases](https://github.com/FuturePrayer/optical/releases) 下载对应平台的预编译二进制:

| 二进制 | 用途 | Windows | Linux |
|--------|------|---------|-------|
| `optical` | 隧道节点 | `optical-x86_64-pc-windows-msvc.exe` | `optical-x86_64-unknown-linux-musl` |
| `optical-center` | 配置中心(含 Web UI) | `optical-center-x86_64-pc-windows-msvc.exe` | `optical-center-x86_64-unknown-linux-musl` |

> Linux 发布版使用 musl 静态链接,零 glibc 依赖,可在任意发行版运行。Windows 版静态链接 CRT,不依赖 `VCRUNTIME140.dll`。
...
# 只编译节点二进制(轻量,不需要 Node.js)
cargo build --release -p optical

# 只编译配置中心(会自动构建前端,需要 Node.js + npm)
cargo build --release -p optical-center

# 全量编译
cargo build --release
```

`optical-center` 编译时会自动触发 `npm install && npm run build` 构建前端,产物通过 `rust-embed` 编译期嵌入二进制。设 `OPTICAL_SKIP_WEBUI=1` 可跳过前端构建(用上次构建的产物,纯 Rust 改动时加速)。

> **极简部署**:如果只需要隧道转发功能(不需要配置中心),只编译/下载 `optical` 即可——它不含任何配置中心代码。

## 快速开始

### 1. 一键初始化(推荐)

`optical init` 会自动生成 ML-DSA-65 密钥对、随机 PSK,并从模板渲染配置文件。通过 `--template` 选择节点角色:

```bash
# 独立运行节点(默认):本地配 forwarders/tunnel_listen
optical init
...
| 模板 | 命令 | 生成的配置 |
|------|------|-----------|
| `standalone`(默认) | `optical init` | 含 forwarders + tunnel_listen,独立运行 |
| `managed-node` | `optical init --template managed-node` | 含 center: 块,连配置中心受纳管 |
| `center` | `optical-center init --template center` | 含 center_listen + Web 管理后台配置(仅 optical-center 可用) |
...
用配置中心管理多个节点的完整流程,详见 **[docs/config-center.md](docs/config-center.md)**。最简步骤:

```bash
# 1. 初始化并启动配置中心(含 Web UI)
optical-center init --template center
# 编辑 config.yml,改 center_admin_token
optical-center run --config config.yml

# 2. 浏览器打开 http://<center-ip>:9100,输入 token 登录

# 3. 在每台节点上初始化被纳管节点
optical init --template managed-node
# 编辑 config.yml:填 center.address 和 center.psk
optical run --config config.yml

# 4. 节点自动连中心 → 在 Web UI 的"待审批"页批准 → 下发 forwarder 配置
```
...
| 命令 | 说明 |
|------|------|
| `optical init [--template standalone\|managed-node] [--system\|--user\|--config-dir <dir>] [--force]` | 一键初始化:生成密钥 + PSK + 配置文件。`--template` 选择角色模板 |
...
| `optical update [--check] [--force] [--restart]` | 检查并更新到最新版本 |

### `optical-center`(配置中心)

`optical-center` 具备 `optical` 的全部命令(双重身份),额外支持配置中心模板:

| 命令 | 说明 |
|------|------|
| `optical-center init [--template standalone\|managed-node\|center] ...` | 初始化,`--template center` 生成配置中心配置 |
| `optical-center run --config <path>` | 运行(双重身份:节点 + 配置中心) |
| 其余命令(`install`/`run`/`keygen`/`status`/`update` 等) | 与 `optical` 相同 |
...
| 平台 | 节点二进制 | 配置中心二进制 |
|------|-----------|---------------|
| Windows x86_64 | `optical-x86_64-pc-windows-msvc.exe` | `optical-center-x86_64-pc-windows-msvc.exe` |
| Linux x86_64 | `optical-x86_64-unknown-linux-musl` | `optical-center-x86_64-unknown-linux-musl` |

每个二进制旁都发布同名的 `.sha256` 校验文件。`optical update` 按自身二进制名(`optical` vs `optical-center`)拉取对应的 asset,两个二进制各自只更新自己。
...
optical init --template managed-node --config-dir .
...
- **配置中心 PSK**(`center_psk`):中心管理域的独立 PSK,与隧道 PSK 分离。节点连配置中心时使用此 PSK 握手。
- **ML-DSA-65**:用于握手签名认证,防止中间人攻击。公钥的 SHA-256 指纹作为节点的永久身份(node_id)。
- **ML-KEM-768**:用于密钥交换,提供后量子安全性。
- **ChaCha20-Poly1305**:帧级 AEAD 加密,每帧含计数器防重放。
- 管理 API 仅监听 `127.0.0.1`,仅本机可访问。配置中心 Web 管理后台(`center_admin_listen`)如需远程访问应配置 `center_admin_token` 认证。
- **自更新完整性**:`update` 下载二进制后强制 SHA256 校验,不匹配则拒绝替换。
