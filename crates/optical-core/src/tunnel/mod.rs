    ...
    R: AsyncRead + Unpin + Send,
{
    ...
    loop {
        ...
        let (stream_id, counter, frame_type_raw, payload_len) = parse_header(&header);
        ...
        // Convert the raw frame-type byte into a typed FrameType. Unknown types
        // (including config-center frames 0x0B-0x0E, which are never sent over
        // a tunnel connection) are silently skipped — this is the forward-
        // compatibility path documented in AGENTS.md ("protocol compatibility").
        // Skipping the already-decrypted plaintext is safe: we simply drop it.
        let frame_type = match FrameType::from_u8(frame_type_raw) {
            Ok(ft) => ft,
            Err(_) => {
                tracing::trace!(frame_type = frame_type_raw, "skipping unknown frame type");
                continue;
            }
        };
        ...
        match frame_type {
            ...
            // Config-center frames (NodeRegister/ConfigPush/StatusReport/ConfigAck)
            // are never sent over a tunnel multiplexer connection — they live on
            // the separate center session. If one somehow arrives here, ignore it
            // rather than erroring (defensive; see AGENTS.md compatibility note).
            FrameType::NodeRegister
            | FrameType::ConfigPush
            | FrameType::StatusReport
            | FrameType::ConfigAck => {
                tracing::trace!(frame_type = ?frame_type, "ignoring center frame on tunnel connection");
            }
        }
    }
    inner.alive.store(false, Ordering::SeqCst);
