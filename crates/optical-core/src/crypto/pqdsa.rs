...
impl DsaKeyPair {
    ...
    /// Encode the verifying (public) key as raw bytes (1952 bytes).
    pub fn encode_vk(&self) -> Vec<u8> {
        self.verifying_key().encode().to_vec()
    }

    /// The node's permanent identity: SHA-256 of the 1952-byte verifying key,
    /// hex-encoded (64 chars). Stable across restarts and unique per key pair.
    pub fn node_id(&self) -> String {
        fingerprint_vk(&self.encode_vk())
    }
}

/// Compute a node ID from a raw verifying key (1952 bytes): SHA-256 → hex.
///
/// This is the canonical, stable identifier for a node. The same key pair
/// always yields the same node ID; it does not reveal the private key.
pub fn fingerprint_vk(vk_bytes: &[u8]) -> String {
