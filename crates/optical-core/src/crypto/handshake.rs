...
///
/// `peer_node_id` is the SHA-256 fingerprint of the peer's ML-DSA-65
/// verifying key — a stable, unique identifier for the peer node. Populated
/// from the `dsa_pubkey` exchanged in ClientHello/ServerHello (which is part
/// of the handshake wire format, so this field requires no protocol change).
#[derive(Clone)]
pub struct HandshakeResult {
    pub role: HandshakeRole,
    pub send_cipher: crate::crypto::aead::AeadCipher,
    pub recv_cipher: crate::crypto::aead::AeadCipher,
    /// SHA-256 fingerprint (hex) of the peer's ML-DSA-65 verifying key.
    /// None only if the handshake was constructed without a peer key (never
    /// happens in practice — both ClientHello and ServerHello always carry it).
...
impl HandshakeState {
    ...
    fn build_result(&self, session_keys: crate::crypto::kdf::SessionKeys) -> HandshakeResult {
        ...
        // Derive the peer node ID from the peer's verifying key, if known.
        // `peer_dsa_pubkey` is always set after a successful handshake (both
        // ClientHello and ServerHello carry dsa_pubkey and are decoded before
        // Finished verification).
        let peer_node_id = self
            .peer_dsa_pubkey
            .as_ref()
            .map(|vk| crate::crypto::pqdsa::fingerprint_vk(vk.encode().as_slice()));
        HandshakeResult {
            role: self.role,
            send_cipher,
            recv_cipher,
